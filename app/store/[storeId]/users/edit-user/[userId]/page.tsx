...
export default function EditUser({
  ...
  const { firstName, lastName, email, phoneNumber, status } = userData;
  ...
  useEffect(() => {
    const role = roles.find((role) => role.id === userData?.role?._id) || {
      id: "",
      name: "",
    };
  ...
  async function handleUpdateUser() {
    ...
    setIsLoading(true);
    try {
      const response = await putData("/api/users", {
        firstName,
        lastName,
        phoneNumber,
        email,
                ...
                <Dropdown
                  ...
                  onClick={(value) =>
                    setUserData((userData) => ({
                      ...userData,
                      status: value?.name?.toLowerCase() === "active",
                    }))
                  }
                  options={[
                    { id: "1", name: "Active" },
